...
OregonH.Event.eventTypes = [
  {
    ...
    stat: "crew",
    value: -3,
    text: "Food intoxication. Casualitie: ",
  },
  {
    type: "STAT-CHANGE",
    notification: "negative",
  ...
  },
  {
    ...
    stat: "oxen",
    value: -1,
    text: "Ox flu outbreak. Casualities: ",
  },
  {
    type: "STAT-CHANGE",
...
OregonH.Event.stateChangeEvent = function (eventData) {
  ...
  if (eventData.value + this.carvan[eventData.stat] >= 0) {
    this.carvan[eventData.stat] += eventData.value;
    this.ui.notify(
...
OregonH.Event.attackEvent = function (eventData) {
  let firepower = Math.round(
    (0.7 + 0.6 * Math.random()) * OregonH.ENEMY_FIREPOWER_AVG
  );
  let gold = Math.round(
    (0.7 + 0.6 * Math.random()) * OregonH.ENEMY_GOLD_AVERAGE
  );
